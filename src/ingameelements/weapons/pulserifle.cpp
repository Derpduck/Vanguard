...
void Pulserifle::init(uint64_t id_, Gamestate &state, EntityPtr owner_)
{
    ...
    fthanim.init(herofolder()+"fanthehammerstart/", std::bind(&Pulserifle::firesecondary, this, std::placeholders::_1));
...
void Pulserifle::firesecondary(Gamestate &state)
{
    ...
    if (clip > 0 and state.engine.isserver)
    {
        if (isfthing)
        {
            fthanim.init("heroes/mccree/fanthehammerloop/", std::bind(&Pulserifle::wantfiresecondary, this, std::placeholders::_1));
        }
        else
        {
            fthanim.init("heroes/mccree/fanthehammerstart/", std::bind(&Pulserifle::wantfiresecondary, this, std::placeholders::_1));
            isfthing = true;
        }
    }
}
...
void Pulserifle::fireultimate(Gamestate &state)
{
    ...
    if (deadeyetargets.size() > 0)
    {
        ...
        deadeyeanim.init("heroes/mccree/fanthehammerloop/", std::bind(&Pulserifle::fireultimate, this, std::placeholders::_1));
    }
    else
    {
