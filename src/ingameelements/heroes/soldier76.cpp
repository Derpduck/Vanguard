...
void Soldier76::init(uint64_t id_, Gamestate &state, EntityPtr owner_)
{
    ...
    ulting.init(6, std::bind(&Soldier76::resetafterult, this, std::placeholders::_1));
    ulting.active = false;
...
void Soldier76::drawhud(Renderer &renderer, Gamestate &state)
{
...
void Soldier76::resetafterult(Gamestate &state)
{
    ulting.active = false;
    ultwalkanim.reset();
    ultcooldown.reset();
